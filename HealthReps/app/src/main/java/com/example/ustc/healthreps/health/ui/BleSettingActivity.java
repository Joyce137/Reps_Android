...
/**
 * Created by CaoRuijuan on 4/6/16.
 */
...
public class BleSettingActivity extends AppCompatActivity implements View.OnClickListener,
    ...
    private final static String TAG = BleSettingActivity.class
            .getSimpleName();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!mBluetoothLeService.initialize()) {//引用服务类的初始化
            Toast.makeText(BleSettingActivity.this, "未初始化", Toast.LENGTH_LONG).show();
        }
        if(!SampleGattAttributes.connectedState) {
            if (!mBluetoothLeService.connect(MainActivity.mDeviceAddress)) {
                Toast.makeText(BleSettingActivity.this, "未连接上", Toast.LENGTH_LONG).show();
                rec.setVisibility(View.VISIBLE);
                System.out.println("断线重连显示");
            }
            else {
                Toast.makeText(BleSettingActivity.this, "已经连接", Toast.LENGTH_LONG).show();
            }
        }
        else {
            Toast.makeText(BleSettingActivity.this, "已经连接", Toast.LENGTH_LONG).show();
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.ly_setting_time:
                istestxinlv = false;
                timeSyn();
                Toast.makeText(BleSettingActivity.this, "时间同步", Toast.LENGTH_LONG).show();
                break;
    ...
    private void  reconnectsettle(){
        if(!SampleGattAttributes.connectedState) {
            if (!mBluetoothLeService.connect(MainActivity.mDeviceAddress)) {
                Toast.makeText(BleSettingActivity.this, "重连失败", Toast.LENGTH_LONG).show();
            }}
        else{
            SampleGattAttributes.connectedState=true;rec.setVisibility(View.GONE);
            Toast.makeText(BleSettingActivity.this, "重连成功", Toast.LENGTH_LONG).show();
            autodisconnect();
        }
    }
    //选择测心率的周期
    public void xinlv() {
        ...
        android.support.v7.app.AlertDialog.Builder sinChosDia = new android.support.v7.app.AlertDialog.Builder(BleSettingActivity.this);
    ...
    private final BroadcastReceiver mGattUpdateReceiver01 = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (BluetoothLeService.ACTION_GATT_CONNECTED.equals(action)) {
                ...
                if (istestxinlv) {
                    if (istestxinlv_open) {
                        Toast.makeText(BleSettingActivity.this, "打开成功！", Toast.LENGTH_LONG).show();
                    ...
                    if (istestxinlv_close) {
                        Toast.makeText(BleSettingActivity.this, "关闭成功！", Toast.LENGTH_LONG).show();
                        SampleGattAttributes.checkflag=false;
                        checkflag2=true;
                        switchCompat.setChecked(false);
                    }
                } else {
                    Toast.makeText(BleSettingActivity.this, "修改成功！", Toast.LENGTH_LONG).show();
                }
            }
            //没有连接上BLE
                ...
                System.out.println("断线重连显示，并关闭判断中");
                    ...
                    .equals(action)) {
                Toast.makeText(BleSettingActivity.this, "服务获取", Toast.LENGTH_LONG).show();
                ...
                if (intent.getStringExtra("DataType").equals("SevenDayData")) {
                    ...
                    Toast.makeText(BleSettingActivity.this, "电量查询：" + SampleGattAttributes.dataGetter(intent.getByteArrayExtra("data"), 0, 1) + "%", Toast.LENGTH_LONG).show();
